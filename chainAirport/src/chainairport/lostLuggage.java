 ...
 * @author Jack
 */
public class lostLuggage implements helpDesk{
    helpDesk next; //This link of the chain holds a reference to the next link
    final int level = 7; //This is the sixth link of the chain
/**
 * This method overrides the chainNumber method in the helpDesk interface. The 
 * method will open up a screen with the heading lost luggage, which will have more 
 * details about lost luggage.
 * @param level 
 */
    @Override
    public void chainNumber(int level) {
        if (this.level == level)
        {
            System.out.println("Lost Luggage Screen"); //If this link handles the object, this line will be displayed
        }
        else 
        {
          next.chainNumber(level); //If this link does not handle the object, it passes the object to the next link
          System.out.println("lostLuggage does not handle this request");
        }
    }
    //This method will create a new link in the chain
    /**
