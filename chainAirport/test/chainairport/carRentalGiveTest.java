
package chainairport;

import org.junit.After;
import org.junit.AfterClass;
import org.junit.Before;
import org.junit.BeforeClass;
import org.junit.Test;
import static org.junit.Assert.*;

/**
 *
 * @author Jack
 */
public class carRentalGiveTest {
    

    /**
     * Test of chainNumber method, of class carRentalGive.
     */
    @Test
    public void testChainNumber() {
        System.out.println("chainNumber");
        int level = 1;
        carRentalGive instance = new carRentalGive();
        instance.chainNumber(level);

    }

    /**
     * Test of setNextElement method, of class carRentalGive.
     */
    @Test
    public void testSetNextElement() {
        System.out.println("setNextElement");
        helpDesk next = null;
        carRentalGive instance = new carRentalGive();
        instance.setNextElement(next);
    }
    
}
